...
// --- CONFIGURATION ---
#define TRAIL_COLOR vec4(sRGBToLinear(iCurrentCursorColor.rgb), iCurrentCursorColor.a) // for custom color: vec4(0.2, 0.6, 1.0, 0.5); (wrap in sRGBToLinear for correct brightness)
...
float antialising(float distance, float blurAmount) {
  return 1. - smoothstep(0., normalizeCoord(vec2(blurAmount, blurAmount), 0.).x, distance);
}
